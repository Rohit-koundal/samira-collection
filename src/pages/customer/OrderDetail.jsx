    ...
    {loading ? <OrderState loading /> : error ? <OrderState title="Unable to open this order" error={error} retry={orderId ? refresh : undefined}><button className="sc-orders__text" onClick={() => navigate('/orders')}>Back to orders</button></OrderState> : order && <>
      ...
      <div className="sc-order-detail__grid"><div className="sc-order-detail__primary">
        ...
        {returns?.requests?.length > 0 && <section className="sc-order-panel"><header><h2>Returns & exchanges</h2></header>{returns.requests.map((request) => {
          ...
          return <article className="sc-order-return" key={request._id}><div><h3>{item?.name || 'Order item'} · {request.type === 'exchange' ? 'Exchange' : 'Return'}</h3><span className="sc-order-status sc-order-status--return">{request.status}</span></div><p>{request.quantity} unit(s) · {request.reason}</p><p>Requested {orderDate(request.createdAt, true)}</p>{request.exchangeSize && <p>Replacement: {request.exchangeSize} {request.exchangeColor}</p>}{request.pickupScheduledAt && <p>Pickup scheduled {orderDate(request.pickupScheduledAt)}</p>}{request.adminComment && <p>{request.adminComment}</p>}<p className="sc-orders__muted">Request #{String(request._id).slice(-8).toUpperCase()}</p>{request.shipment && <DeliveryTracking returnId={request._id} />}</article>;
        })}</section>}
