...
exports.stats = async (req, res) => {
  const [products, orders, customers, coupons, returns] = await Promise.all([
  ...
  res.json({ products, orders, customers, coupons, returns });
};
exports.recentOrders = async (req, res) => res.json(await Order.find().sort('-createdAt').limit(10));
