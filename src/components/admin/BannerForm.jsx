...
export default function BannerForm({ onSaved }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-4 rounded-2xl bg-white p-5 shadow-sm md:grid-cols-2">
      ...
      <div className="md:col-span-2"><ImageUploader value={form.image ? [{ url: form.image }] : []} onChange={(images) => update('image', images[0]?.url || '')} compressAboveMb={2} maxUploadMb={20} targetSizeMb={0.5} /></div>
