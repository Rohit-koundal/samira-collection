...
const emptyCoupon = { code: '', type: 'Percentage', discountValue: '', minOrderAmount: 0, maxDiscountAmount: '', expiryDate: '', usageLimit: '', isActive: true };
...
export default function CouponForm({ onSaved }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-4 rounded-2xl bg-white p-5 shadow-sm md:grid-cols-3">
      ...
      <button disabled={saving} className="h-12 rounded-xl bg-wine text-sm font-black text-white disabled:opacity-60 md:col-span-3">{saving ? 'Saving...' : 'Save Coupon'}</button>
    </form>
  );
}
