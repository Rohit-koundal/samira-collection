    ...
    <section className="container-page py-8 md:py-12">
      ...
      <div className="hide-scrollbar flex gap-3 overflow-x-auto md:grid md:grid-cols-4 lg:grid-cols-8">
        {categories.map((category, index) => (
          <button key={category._id || category.id} onClick={() => navigate(`/products?category=${category._id || ''}`)} className="min-w-[136px] rounded-3xl bg-white p-4 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
            {category.image ? <img src={normalizeImageUrl(category.image)} alt="" className="h-20 w-full rounded-2xl object-cover" /> : <div className="grid h-20 place-items-center rounded-2xl bg-gradient-to-br from-blush to-[#f7e4c7] text-3xl font-black text-wine">SC</div>}
