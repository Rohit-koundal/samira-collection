...
export default function Login() {
  ...
  return (
    <section className="grid min-h-[70vh] place-items-center px-4 py-10">
      <form onSubmit={(event) => { event.preventDefault(); login({ email, password }); }} className="w-full max-w-md rounded-3xl bg-white p-7 shadow-xl">
        ...
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-3 h-12 w-full rounded-xl border border-slate-200 px-4 text-sm font-semibold" placeholder="Password" />
