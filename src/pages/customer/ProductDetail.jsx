    ...
    <section className="bg-[#f5f5f6] pb-36 md:bg-ivory md:pb-10 md:pt-8">
      ...
      <div className="mx-auto max-w-6xl md:grid md:grid-cols-[0.95fr_1fr] md:gap-8 md:px-6">
          ...
          >
            {selectedImage ? (
              <button type="button" onClick={() => setOpenGallery(true)} className="block w-full bg-[#f6efe8]">
                <img src={selectedImage} alt={product.name} className="h-[290px] w-full object-contain bg-[#f6efe8] sm:h-[430px] md:h-[620px] md:object-cover" />
              </button>
            ) : (
            ...
            <div className="absolute bottom-3 right-3 min-w-[96px] rounded-xl bg-white px-3 py-2 text-center text-[10px] font-black shadow md:bottom-4 md:right-4 md:min-w-[106px] md:text-xs">
        ...
        <div className="space-y-3 px-3 pt-3 md:space-y-7 md:px-0 md:pt-0">
          <div className="rounded-[14px] bg-white px-3 py-3 md:rounded-none md:bg-transparent md:px-0 md:py-0">
            <div className="relative pr-12">
            <p className="text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500 md:text-base md:tracking-[0.12em]">{product.brand || 'Samira Collection'}</p>
            <h1 className="mt-1 text-[13px] font-semibold leading-[1.35] text-slate-900 md:text-[22px]">
              {product.name}
            </h1>
            <button
              type="button"
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
              <span className="text-[15px] font-bold text-charcoal md:text-2xl">Rs. {product.price}</span>
              <span className="text-[10px] text-slate-400 line-through">Rs. {product.originalPrice}</span>
              <span className="text-[10px] font-bold text-rose">({product.discountPercentage}% OFF)</span>
            </div>
            <div className="mt-1 flex items-center gap-2">
              <span className="rounded-full bg-[#fff0f4] px-2 py-1 text-[8px] font-bold text-rose">4.0</span>
              <span className="text-[8px] font-medium text-slate-500">{product.numReviews || reviews.length || 0} Ratings</span>
            </div>
          </div>

          <div className="rounded-[14px] bg-white p-3 md:rounded-2xl">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[8px] font-bold uppercase tracking-[0.08em] text-rose">Mega Deal</p>
                <p className="mt-1 text-[14px] font-bold text-charcoal md:text-xl">Get at Rs. {dealPrice || product.price}</p>
                <p className="mt-1 text-[8px] text-slate-500">With Coupon + Bank Offer</p>
              </div>
              <span className="rounded-full bg-emerald-500 px-2.5 py-1 text-[8px] font-bold text-white md:text-sm">Extra Rs. {Math.max(0, product.price - dealPrice)} Off</span>
            </div>
          ...
          <section className="rounded-[14px] bg-white p-3 md:rounded-none md:bg-transparent md:p-0">
            <div className="flex items-center justify-between">
              <h2 className="text-[10px] font-semibold text-charcoal md:text-xl">Size</h2>
              <button type="button" onClick={() => setOpenSizeChart(true)} className="text-[9px] font-semibold text-rose">Size Chart</button>
            </div>
            <div className="mt-2 flex flex-wrap gap-2">
              {(product.sizes?.length ? product.sizes : ['Free Size']).map((item) => (
                <button key={item} type="button" onClick={() => setSize(item)} className={`grid h-7 min-w-7 place-items-center rounded-full border px-2.5 text-[9px] font-semibold md:min-w-24 md:rounded-2xl md:px-5 md:py-4 ${size === item ? 'border-[#7a1f36] bg-[#7a1f36] text-white' : 'border-slate-200 bg-white text-charcoal'}`}>{item}</button>
              ))}
            </div>
            {product.colors?.length > 0 && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-semibold text-charcoal">Color: {color}</span>
          ...
          <section className="rounded-[14px] bg-white p-3 md:space-y-2 md:rounded-none md:bg-transparent md:p-0 md:pt-2">
            <h2 className="text-[10px] font-semibold text-charcoal md:text-xl">Check Delivery</h2>
            <div className="mt-2 flex gap-2">
              <input value={deliveryPin} onChange={(event) => setDeliveryPin(event.target.value.replace(/\D/g, '').slice(0, 6))} className="h-9 flex-1 rounded-[8px] border border-slate-300 px-3 text-[10px] outline-none focus:border-rose md:h-12 md:rounded-xl md:px-4 md:text-base" placeholder="Enter PIN Code" />
              <button type="button" className="h-9 rounded-[8px] bg-[#7a1f36] px-4 text-[10px] font-bold text-white md:h-12 md:rounded-xl">Check</button>
            </div>
      ...
      <div className="mx-auto mt-9 max-w-6xl px-4 md:px-6">
        <div className="rounded-[14px] border border-slate-200 bg-white p-4 md:rounded-3xl md:p-5">
          {[
            ...
            <button key={label} type="button" onClick={() => navigate(`/products?category=${product.categoryId || ''}`)} className="flex w-full items-center justify-between gap-3 border-b border-slate-100 py-3 text-left text-[10px] font-semibold text-[#1f2a44] last:border-b-0 md:text-lg md:font-black">
              {label.trim()}
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 bg-white px-3 py-2 shadow-[0_-8px_20px_rgba(15,23,42,0.08)] md:hidden">
        <div className="grid grid-cols-2 gap-2">
          <button disabled={isOutOfStock} onClick={buyNow} className="h-10 rounded-[8px] border border-[#7a1f36] bg-white px-3 text-[11px] font-bold text-[#7a1f36] disabled:opacity-50">
            Buy Now
          </button>
          <button disabled={isOutOfStock} onClick={add} className={`flex h-10 items-center justify-center gap-2 rounded-[8px] px-3 text-[11px] font-bold text-white disabled:opacity-50 ${cartItem ? 'bg-emerald-600' : 'bg-[#7a1f36]'}`}>
