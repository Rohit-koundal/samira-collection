...
Gemini 2.5 Flash offers a free API tier with usage limits and a separate usage-billed paid tier. No billing account is enabled by this application. See [Google's current pricing](https://ai.google.dev/gemini-api/docs/pricing#gemini-2.5-flash) and [video input documentation](https://ai.google.dev/gemini-api/docs/video-understanding).
...
The frame tests use real encoded videos to check blur, darkness, duplicate removal and late product appearances beyond the first 48 seconds. Integration checks cover selected covers and view labels through draft creation, publishing and subsequent product edits, including protection of saved drafts against rechecks. Optional AI request and failure handling use fixture responses; no live Gemini account was used in verification.
