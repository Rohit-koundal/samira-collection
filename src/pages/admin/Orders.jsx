...
const taskLabels = { pending: 'New orders', packing: 'To pack', todayPacking: "Today's packing", dispatch: 'To dispatch', transit: 'In transit', exceptions: 'Delivery issues', returns: 'Open returns', collection: 'Delivered COD awaiting collection' };
...
export default function Orders({ route = '' }) {
  ...
  const cancelOrder = async () => {
    ...
    await api.put(`${apiBase}/orders/${cancelTarget._id}/status`, { orderStatus: 'Cancelled', revision: Number(cancelTarget.revision || 0), note: reason });
    setCancelTarget(null); setCancelReason(''); await refresh();
  };
  ...
  const exportVisible = () => {
    ...
    const rows = [['Invoice', 'Order ID', 'Customer', 'Phone', 'Amount', 'Payment', 'Order status', 'Delivery', 'AWB'], ...filtered.map(order => [order.invoiceNumber, order._id, order.user?.name || order.shippingAddress?.fullName, order.user?.phone || order.shippingAddress?.mobile, order.finalAmount, `${order.paymentMethod} ${order.paymentStatus}`, order.orderStatus, order.shipment?.status || 'WAITING', order.shipment?.awb || ''])];
  ...
  return <section className="order-workspace space-y-5">
    ...
    <DataTable loading={loading} error={loadError} onRetry={load} emptyTitle="No orders found" title="Order queue" note="Actions are limited to the next safe workflow step." minWidth={1100}
      ...
      rows={filtered.map(order => <tr key={order._id} className={selected.includes(order._id) ? 'is-selected' : ''}>
        ...
        <td className="px-4 py-4"><strong>₹{Number(order.finalAmount || 0).toLocaleString('en-IN')}</strong></td>
        <td className="px-4 py-4"><StatusBadge value={order.paymentStatus} /><p className="admin-note mt-1">{order.paymentMethod}</p></td>
        <td className="px-4 py-4"><StatusBadge value={order.orderStatus} /><p className="admin-note mt-2">{order.shipment?.status ? order.shipment.status.replaceAll('_', ' ') : 'Waiting for shipment'}</p>{order.shipment?.awb && <p className="admin-note">AWB {order.shipment.awb}</p>}</td>
        <td className="px-4 py-4"><OrderWorkflowActions compact order={order} busy={busyOrder === order._id || busyOrder === 'bulk'} onStatus={updateOrder} onCancel={target => { setCancelTarget(target); setCancelReason(''); }} onCollectCod={setCodTarget} returnHref={seller ? `${apiBase}/orders/detail?id=${order._id}` : `/admin/returns?orderId=${order._id}`} /><a href={`${apiBase}/orders/detail?id=${order._id}`} className="order-view-link">View complete order →</a></td>
      </tr>)} />
    ...
    <ConfirmModal open={Boolean(cancelTarget)} title="Cancel this order?" message="Cancellation restores reserved inventory. It is unavailable after courier handover." confirmLabel="Cancel order" onClose={() => setCancelTarget(null)} onConfirm={cancelOrder}><label className="admin-field">Cancellation reason<textarea className="admin-field__control min-h-24" maxLength={300} value={cancelReason} onChange={event => setCancelReason(event.target.value)} placeholder="Reason recorded in the order timeline" /></label></ConfirmModal>
